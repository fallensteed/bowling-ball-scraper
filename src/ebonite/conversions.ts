...
import { eboniteURL } from "./urls.js";
import { BallSpecs } from "../common/interfaces";
import { children } from "cheerio/lib/api/traversing.js";

export const getEboniteBallDetails = async (
	ball: BallModel
): Promise<BallModel> => {
	if (ball.url) {
		const eboniteBallDetailRawData = await getRawData(ball.url);
		const $ = cheerio.load(eboniteBallDetailRawData);
		const specs: BallSpecs[] = [];

		// get description
		const description = $(".p1").first().text().trim();
